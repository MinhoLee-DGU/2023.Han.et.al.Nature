## scanpy UMAP
sample.info = read.csv('../integ_Neg_Han/sample.info.csv')
umap = read.csv('../integ_Neg_Han/umap.csv')
...
## load Negretti et al. epithelium seurat obj
neg_epi = readRDS('../neg_epi.rds')
neg_epi@meta.data$projcet = 'Negretti et al.'
neg_epi@meta.data$epi_celltype = Idents(neg_epi)
...
## merge data
seurat.list <- SplitObject(all, split.by="Age")
...
# Cell ids
write.csv(Cells(seurat), file = "../integ_Neg_Han/cellID_obs.csv", row.names = FALSE)
# Clusters
write.csv(select(seurat@meta.data, epi_celltype), '../integ_Neg_Han/clusters.csv')
# Cluster colors
...
write.csv(cell.colors, '../integ_Neg_Han/cell.colors.csv')
# Age
write.csv(select(seurat@meta.data, Age), '../integ_Neg_Han/Age.csv')
# project
write.csv(select(seurat@meta.data, project), '../integ_Neg_Han/project.csv')
